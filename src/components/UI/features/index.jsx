...
export default function Features({ dataList }) {
  return (
    <section id="edu-coozin" className="tt:py-12 py-4 tt:mt-10">
      <div
        className={`${cls.container} container tt:h-[800px] items-center tt:gap-6 gap-10 flex tt:flex-row flex-col justify-between`}
      >
        ...
        <div className={`${cls.features__box} tt:py-[80px]`}>
          ...
          <ul className="notel:mt-[56px] mt-5 flex flex-col gap-[20px]">
            {dataList?.edu_coozin_content &&
              dataList?.edu_coozin_content?.map((e,i) => (
                <li key={i}>
                  <div className="flex items-center gap-2">
                    <Image
                      ...
                      alt="right"
                    />
                    <h3 className="text-[#374151] leading-5 text-xl font-semibold">
                      {e?.title}
                    </h3>
                  </div>
                  <p className="text-[#374151] mt-2 font-normal text-base">
                    {" "}
                    {e?.subtitle}
                  </p>
                </li>
